...
extern const void* GetSkillIconGfx(int skill_id);
extern int GetSkillDescMsg(int skill_id);
...
void SPD_init(struct SPD_ProcStateMain* proc) {
	...
	CopyTileGfxForObj((void*)GetSkillIconGfx(proc->skill), (void*)0x6013380, 2, 2);
  
	// Prepare skill name.
	GetStringFromIndex(GetSkillDescMsg(proc->skill));
	
	int i = 0;
	while( (gCurrentTextString[i] != ' ') ) // 0x3A = ":", we cut off everything past colon.
	{
		if( 0x3A == gCurrentTextString[i] )
			break;
  ...
  *(u32*)((u32)font+8) = 0x8004269;    // Draws glyphs by overwriting pixels instead of adding.
  ...
  Decompress(SPD_ProcDisplayTiles, (void*)0x6000020);
...
void SPD_Map_init(struct SPD_Map_ProcStateMain* proc) {
  ...
  CpuFastCopy((void*)GetSkillIconGfx(proc->skill), (void*)0x6000C00, 0x80);
