...
void SPD_init(struct SPD_ProcStateMain* proc) {
	...
	extern int GetStringTextWidth(const char*);
	
	char* str_name;
  
	CopyTileGfxForObj((void*)proc->icon, (void*)0x6013380, 2, 2);
	...
	*(u32*)((u32)font+8) = 0x8004269;    // Draws glyphs by overwriting pixels instead of adding.
	...
	Text_SetXCursor(&proc->textHandle, (int)Text_GetStringTextCenteredPos(SKILLDISPLAY_WIDTH*8, str_name));
	Decompress(SPD_ProcDisplayTiles, (void*)0x6000020);
	Text_DrawString(&proc->textHandle, str_name);
...
void SPD_Map_init(struct SPD_Map_ProcStateMain* proc) {
	...
	CpuFastCopy(proc->icon, (void*)0x6000C00, 0x80);
...
void SPD_Map_loop(struct SPD_Map_ProcStateMain* proc) {
	...
	for (i = 0; i < k; i++)
		for (j = 0; j < 2; j++)
			gBg0MapBuffer[(proc->y+j+1)*0x20 + proc->x+l*(10+m-i+offset)] 
				= j * 2 - i * l + 0x4061 - m;
